...
public class ExpressionEvaluation{
	...
	//private static Pattern idIntPattern = Pattern.compile("_id(att)?\\d+");
	...
	public boolean evaluate(String exp, int[] candidate){
		...
		boolean res1 = evaluateInnerv2(exp, candidate);
		
/*		int[] assignment = getAssignment(exp, candidate);
		String generalExp = generaliseExpression(exp, assignment.length);
		
		//System.out.println(evaluation);
		boolean res2 = evaluateInner(generalExp, assignment);
		if((res1 && !res2) || (!res1 && res2)){
			System.err.println("- - - Discrepance in eval: "+exp+" 1: "+res1+", 2: "+res2);
			System.err.println(getVectorAsString(candidate));
		}*/
		return res1;
	...
	private boolean evaluateInnerv2(String exp, int[] assignment){
		//System.out.println("Original: "+exp);
		assignment = getAssignment(exp, assignment);
		exp = preprocess(exp, assignment.length);
		//System.out.println("PreProcessed: "+exp+", "+getVectorAsString(assignment));
		String assignmentKey = getVectorAsString(assignment);
		if(expressionsEvaluated2.containsKey(exp)){
			HashMap<String, Boolean> evaluation = expressionsEvaluated2.get(exp);
			if(evaluation.containsKey(assignmentKey)){
				boolean result = evaluation.get(assignmentKey);
				//System.out.println(exp+", "+assignmentKey+" = "+result);	//
				return result;
			}else{
				boolean result = evaluateRecursivev2(exp, assignment);
			...
			//System.out.println(exp+", "+assignmentKey+" = "+result);		//
			evaluation.put(assignmentKey, result);
			expressionsEvaluated2.put(exp, evaluation);
			return result;
		}
	}
	...
	private boolean evaluateRecursivev2(String exp, int[] assignment){
		Matcher implMatcher = implPattern.matcher(exp);
		if(implMatcher.find()){
			String[] clauses = splitExpression(exp);
			//System.out.println(clauses[0]+"| --> |"+clauses[1]); 		//
			return (!evaluateInnerv2(clauses[0], assignment)) || evaluateInnerv2(clauses[1], assignment);
		}
	...
	private boolean evalContext(String contextExp){
		if(contextEvaluated.containsKey(contextExp)){
			//System.out.println("RT: "+contextExp+" = "+contextEvaluated.get(contextExp));
			...
			//System.out.println("CAlc: "+contextExp+" = "+res);
			return res;
		}
	}
	...
	private String generaliseExpression(String exp, int numberOfIds){
		...
		int newId = 0;
		//System.out.println(exp+" - should have "+numberOfIds+" ids to replace");
		...
		//System.out.println("StrBui: "+result);
...
/*		}else if(isCompoundOrGrouping(exp)){
	...
	public boolean isImplication(String exp){
		// TODO: Make methods for all 3 possible isImplication cases
		Matcher m = implPattern.matcher(exp);
		return m.find();
	}
